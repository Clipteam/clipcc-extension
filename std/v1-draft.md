...
**dependency**：依赖的扩展，键为扩展 ID，值为依赖的版本。版本号有如下格式：`1.2.0` 表示版本只能为 `1.2.0`；`1.2.*` 表示版本号可以为 `1.2.0`、`1.2.1` 等，在匹配中 `*` 可以出现多次，但必须表示版本号中完整的某一段，即 `1.2.3*` 是不合法的，并且 `*` 不能出现在包含 `^` 或 `~` 开头的版本号匹配中；`^1.2.3` 表示版本号大于等于 `1.2.3` 但小于 `2.0.0`；`~1.2.3` 表示版本号大于等于 `1.2.3` 但小于 `1.3.0`。
